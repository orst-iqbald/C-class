...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    {
        curr = backCirListDeque(&stack);
        removeBackCirListDeque(&stack);
        
        if(curr->label == destination->label)
            return 1;
        else
        {
            if(curr->isVisited == 0)
            ...
            for (i = 0; i < curr->numNeighbors; i++)
            {
                ...
                {
                    if(curr->neighbors[i]->isVisited == 0)
                        addBackCirListDeque(&stack, curr->neighbors[i]);
                }
            }
        }
    }
    ...
    while(!isEmptyCirListDeque(&queue))
    {
        ...
        else
        {
            ...
            for (i = 0; i < curr->numNeighbors; i++)
            {
                ...
                {
                    if(curr->neighbors[i]->isVisited == 0)
                        addBackCirListDeque(&queue, curr->neighbors[i]);
                }
            }
        }
    }
